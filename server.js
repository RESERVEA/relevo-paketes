...
class Sala {
  ...
  entra(c) {
    ...
    /* La presencia va a TODOS, no solo al muro: en la puerta de entrada cada
       piloto ve quién más ha entrado y con qué nombre, que es lo que evita que
       dos se pongan el mismo y se echen el uno al otro sin enterarse. */
    this.mandar("todos", this.reparto());
  }
  sale(c) {
  ...
  recibe(c, dato) {
    ...
    /* el muro avisa de que se acabó la carrera: el piloto limpia su pantalla */
    if (m.tipo === "finCarrera" && c.rol === "muro") {
      this.mandar("piloto", { tipo: "finCarrera", en: Date.now() });
      return;
    }
    ...
    /* El muro da el enterado de un aviso del piloto: se lo pasamos a él. */
    if (m.tipo === "recibido" && c.rol === "muro") {
      /* Si el muro dice a quién va dirigido, solo lo recibe ese piloto: el que
         está en el box con el móvil encendido no tiene por qué ver el
         «RECIBIDO» de un aviso que no ha mandado él. */
      const para = String(m.para || "").trim();
      const paq = { tipo: "recibido", texto: m.texto || null, para: para || null, en: Date.now() };
      if (para) {
        const norm = x => String(x || "").trim().toUpperCase();
        for (const o of this.clientes) {
          if (o.rol !== "piloto" || o.ws.readyState !== 1) continue;
          if (norm(o.nombre) !== norm(para)) continue;
          try { o.ws.send(JSON.stringify(paq)); } catch (e) {}
        }
        return;
      }
      this.mandar("piloto", paq);
      return;
    }
    /* El piloto pulsa un botón de su pantalla: llega al muro tal cual. */
    if (m.tipo === "delPiloto" && c.rol === "piloto") {
      this.mandar("muro", { ...m, nombre: c.nombre || m.piloto || null, en: Date.now() });
      return;
    }
    /* ── El estado de cada piloto también va a los demás pilotos ──
       Así cada móvil puede montar por su cuenta la tabla del entrenamiento en
       grupo, sin depender de que haya un ordenador encendido en el box. El
       muro lo sigue recibiendo igual. */
    if (m.tipo === "estado" && c.rol === "piloto") {
      this.mandar("piloto", { ...m, piloto: c.nombre || m.piloto || null, en: Date.now() });
    }
    /* Los ajustes de la tanda que manda el capitán desde su teléfono: van al
       muro, que es quien los aplica y los vuelve a repartir a todos. */
    if (m.tipo === "ajusteEntreno" && c.rol === "piloto") {
      this.mandar("muro", { ...m, piloto: c.nombre || m.piloto || null, en: Date.now() });
      return;
    }
    /* Las órdenes del capitán desde la pista: a todos los demás y al muro. */
    if (m.tipo === "jefe" && c.rol === "piloto") {
      this.mandar("todos", { ...m, piloto: c.nombre || m.piloto || null, en: Date.now() });
      return;
    }
    /* El capitán da la salida desde su móvil: se reparte a todos tal cual. */
    if (m.tipo === "grupo" && c.rol === "piloto") {
      this.mandar("todos", { ...m, piloto: c.nombre || m.piloto || null, en: Date.now() });
      return;
    }
    if (m.tipo === "estado" && c.rol === "piloto") {
      ...
      /* ── Y también a los demás pilotos ──
         En el entrenamiento en grupo cada móvil arma su propia tabla con lo
         que oye de los otros. Si el estado solo llega al muro, cada uno se ve
         solo en la tabla: ni delante, ni detrás, ni mejor de la tanda. Eso es
         lo que pasó en Campillos. */
      for (const o of this.clientes) {
        if (o === c) continue;
        if (o.rol !== "piloto") continue;
        if (o.ws.readyState !== 1) continue;
        try { o.ws.send(JSON.stringify({ ...e, tipo: "estado" })); } catch (err) {}
      }
      return;
    }
